...
http
  .createServer((req, res) => {
    ...
    if (req.method == 'GET' && parsedUrl.pathname == '/') {
      ...
      res.end();
      console.log(`### Get all products list\n${JSON.stringify(list, null, 2)}`);
      return;
    }

    if (req.method == 'GET' && /^\/sellers\/([^\/]+)\/?$/.test(parsedUrl.pathname)) {
      ...
      res.write(JSON.stringify(list, null, 2));
      res.end();
      console.log(`### Get ${seller}'s products list\n${JSON.stringify(list, null, 2)}`);
      return;
    }

    if (req.method == 'GET' && /^\/products\/([^\/]+)\/?$/.test(parsedUrl.pathname)) {
    ...
    if (
      ...
      const seller = decodeURIComponent(parsedUrl.pathname.split('/')[2].split(':')[0]);
      favorite.add(seller);
      res.end();
